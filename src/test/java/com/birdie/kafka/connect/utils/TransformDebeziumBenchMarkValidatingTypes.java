...
public class TransformDebeziumBenchMarkValidatingTypes extends DebeziumJsonDeserializer {

    private static final Logger LOGGER = LoggerFactory.getLogger(TransformDebeziumBenchMarkValidatingTypes.class);
    private final SchemaMapperBenchMark schemaMapper = new SchemaMapperBenchMark(this.schemaTransformer);

    public TransformDebeziumBenchMarkValidatingTypes() {
        super();
    }
    ...
    protected SchemaAndValue transformDebeziumJsonField(SourceRecord record, Field field, String jsonString) {
        ...
        if (unionPreviousMessagesSchema && useProbabilisticFastPath) {
            for (Schema schema: this.getOrCreateListOfKnownSchemasForField(record.topic(), field.name())) {
                try {
                    Object value = this.schemaMapper.mapJsonToSchemaCheckTypes(schema, jsonNode);
                    if (value == null) {
                        return null;
                    }
